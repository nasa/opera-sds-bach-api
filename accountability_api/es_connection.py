...
def get_mozart_es(es_url, logger=None):
    ...
    if MOZART_ES is None:
        ...
        if es_engine == "elasticsearch":
            MOZART_ES = ElasticsearchUtility(es_url, logger=logger)
...
def get_grq_es(logger=None) -> Union[ElasticsearchUtility, OpenSearchUtility]:
    global GRQ_ES
    ...
    if GRQ_ES is None:
        ...
        if aws_es is True or "es.amazonaws.com" in es_url:
            ...
            if es_engine == "elasticsearch":
                # GRQ_ES = ElasticsearchUtility(es_url, logger=logger, product_metadata=product_metadata)
                GRQ_ES = ElasticsearchUtility(es_url, logger=logger)
            elif es_engine == "opensearch":
                GRQ_ES = OpenSearchUtility(
                    es_url,
                    engine=es_engine,
