...
class AWSS3:
    ...
    def from_http_url(self, http_url: str):
        """
        ...
        if not http_url.startswith("http://") and not http_url.startswith("https://"):
            raise ValueError(f"invalid s3 url: {http_url}")
    ...
    def from_s3_url(self, s3_url: str):
    ...
    def get_stream(self):
        ...
        try:
            ...
            raise ValueError(f"error while getting the stream for {self.to_s3_url()}. cause: {str(e)}")
